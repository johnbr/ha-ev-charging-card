...
`,wt=window;wt.customCards=wt.customCards??[],wt.customCards.some(t=>t.type===pt)||(wt.customCards.push({type:pt,name:"EV Charging Card",description:"Compact card showing an EV charging session — power, energy added, SoC, limit, ETA.",preview:!0,documentationURL:"https://github.com/johnbr/ha-ev-charging-card"}),console.info("%c EV-CHARGING-CARD %c v0.1.1 ","color: white; background: #2c5282; font-weight: 700;","color: #2c5282; background: white; font-weight: 700;"));class Et extends at{static async getConfigElement(){return await Promise.resolve().then(function(){return xt}),document.createElement(ft)}static getStubConfig(t,e){const i=e.find(t=>t.startsWith("sensor.")&&/battery/.test(t));return{type:`custom:${pt}`,state_of_charge:i??""}}setConfig(t){if(!t)throw new Error("Invalid configuration");if(!t.state_of_charge)throw new Error("`state_of_charge` entity is required");this._config=t}getCardSize(){return 1}render(){if(!this._config||!this.hass)return q;const t=vt(this.hass,this._config.state_of_charge);if(!t||!t.available)return B`
        ...
        <div class="row">
          ...
          <div class="bar-block">
            ...
            >
              ...
              ${n&&(a||h||l)?B`<div class="metrics">
                    ${a?B`<div class="metric metric-left">${a}</div>`:q}
                    ${h?B`<div class="metric metric-center">ETA ${h}</div>`:q}
                    ${l?B`<div class="metric metric-right">${l}</div>`:q}
                  </div>`:q}
  ...
  `,t([dt({attribute:!1})],St.prototype,"hass",void 0),t([ut()],St.prototype,"_config",void 0),customElements.get(ft)||customElements.define(ft,St);var xt=Object.freeze({__proto__:null,EvChargingCardEditor:St});export{Et as EvChargingCard};
